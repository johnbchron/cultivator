use bevy::{
  ...
  render::{
    ...
    render_resource::{
      ...
      BindingType, BufferBindingType, CachedRenderPipelineId, ColorTargetState,
      ...
      Shader, ShaderDefVal, ShaderStages, ShaderType, TextureFormat,
...
impl Plugin for PixelCamPlugin {
  fn build(&self, app: &mut App) {
    ...
    // Bevy's renderer uses a render graph which is a collection of nodes in a
    // directed acyclic graph. It currently runs on each view/camera and
    // executes each node in the specified order. It will make sure that any
    // node that needs a dependency from another node only runs when that
    // dependency is done.
    //
    // Each node can execute arbitrary work, but it generally runs at least one
    // render pass. A node only has access to the render world, so if you
    // need data from the main world you need to extract it manually or with
    // the plugin like above.
    ...
    // A slot edge tells the render graph which input/output value should be
    // passed to the node. In this case, the view entity, which is the
    // entity associated with the camera on which the graph is running.
    ...
    //
    // Here we want our effect to run after tonemapping and before the end of
    // the main pass post processing
    core_3d_graph
...
struct PixelCamNode {
  // The node needs a query to gather data from the ECS in order to do its
  // rendering, but it's not a normal system so we need to define it
  // manually.
  query: QueryState<
    (
...
impl Node for PixelCamNode {
  ...
  fn input(&self) -> Vec<SlotInfo> {
    ...
    // Currently, every node in bevy uses this pattern, so it's safe to just
    // copy it.
  ...
  //
  // This will run on every view on which the graph is running. If you don't
  // want your effect to run on every camera, you'll need to make sure you
  // have a marker component to identify which camera(s) should run the effect.
  fn run(
    &self,
    ...
    // Get the entity of the view for the render graph where this node is
    // running
    ...
    // It is required to avoid creating a new pipeline each frame, which is
    // expensive due to shader compilation.
    ...
    //
    // Normally, you would create a bind_group in the Queue stage, but this
    // doesn't work with the post_process_write(). The reason it doesn't
    // work is because each post_process_write will alternate the
    // source/destination. The only way to have the correct
    // source/destination for the bind_group is to make sure you get it during
    // the node execution.
    let bind_group =
      render_context
        .render_device()
...
// This contains global data used by the render pipeline. This will be created
// once on startup.
#[derive(Resource)]
  ...
  fn from_world(world: &mut World) -> Self {
    ...
    let pipeline_id = world
      ...
      .queue_render_pipeline(RenderPipelineDescriptor {
        ...
        // All of the following property are not important for this effect so
        // just use the default values. This struct doesn't have the
        // Default trai implemented because not all field can have a default
        // value.
